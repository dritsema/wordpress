...
<?php 
$wp_option = get_option('saml_authentication_options');
...
************************************************
  ...
  <tr valign="top">
    <th scope="row"><label for="idpparameters_attribute">IdP Parameters</label></th> 
    <td><?php echo preg_replace("/\?.+/", "", $this->settings->get_idp()) . '?'; ?> <input type="text" name="idpparameters_attribute" id="idpparameters_attribute_inp" value="<?php echo $this->settings->get_idp_parameters(); ?>" size="40" data-if-empty="error" />
    <br/>
    <span class="setting-description">These parameters will be appended to the IdP URL.  For example, you can enter PartnerSpId=xyz</span>
    </td>
  </tr>
</table> 
<h3>Attributes</h3>
